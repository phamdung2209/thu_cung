...
@php
    ...
    foreach (Session::get('cart') as $key => $cartItem){
        if(get_single_product($cartItem['id'])->added_by == 'admin'){
            array_push($admin_products, $cartItem['id']);
        }
        else{
            $product_ids = array();
            if(isset($seller_products[get_single_product($cartItem['id'])->user_id])){
                $product_ids = $seller_products[get_single_product($cartItem['id'])->user_id];
            }
